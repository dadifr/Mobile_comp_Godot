...
func shoot_burst():
	if bullet_scene == null or player == null: return
	...
	# Il mob si ferma mentre spara la raffica
	velocity = Vector2.ZERO
	
	for i in range(burst_count):
		# Se il mob muore o viene colpito duramente durante la raffica, fermati
		if is_hurt or current_health <= 0: break
		
		# Calcola la direzione verso il player (solo al momento dello sparo)
		...
		b.direction = dir
		b.rotation = dir.angle() # Ruota lo sprite verso la direzione di volo
		
		get_parent().add_child(b)
		
		# Animazione di sparo (se ne hai una, altrimenti usa "idle" o "run")
		# anim.play("attack_ranged") 
		
		# Aspetta il tempo stabilito tra un colpo e l'altro
